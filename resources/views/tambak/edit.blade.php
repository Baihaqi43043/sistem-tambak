...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="mb-0">Edit Data Tambak</h4>
            <a href="{{ route('tambak.index') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left"></i> Kembali
            </a>
        </div>
        <div class="card-body">
            <form action="{{ route('tambak.update', $tambak->id_tambak) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <select class="form-select @error('id_petambak') is-invalid @enderror" id="id_petambak"
                        ...
                        @foreach ($petambaks as $petambak)
                            <option value="{{ $petambak->id_petambak }}"
                                {{ old('id_petambak', $tambak->id_petambak) == $petambak->id_petambak ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nama_tambak') is-invalid @enderror" id="nama_tambak"
                        name="nama_tambak" value="{{ old('nama_tambak', $tambak->nama_tambak) }}" required>
                    @error('nama_tambak')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('lokasi') is-invalid @enderror" id="lokasi"
                        name="lokasi" value="{{ old('lokasi', $tambak->lokasi) }}" required>
                    @error('lokasi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                name="luas_area" value="{{ old('luas_area', $tambak->luas_area) }}" required
                                min="0">
                            @error('luas_area')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                name="kedalaman" value="{{ old('kedalaman', $tambak->kedalaman) }}" required
                                min="0">
                            @error('kedalaman')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('tanggal_pembuatan') is-invalid @enderror"
                        ...
                        value="{{ old('tanggal_pembuatan', $tambak->tanggal_pembuatan->format('Y-m-d')) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status"
                        ...
                        <option value="tidak aktif"
                            {{ old('status', $tambak->status) == 'tidak aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                        <option value="perbaikan" {{ old('status', $tambak->status) == 'perbaikan' ? 'selected' : '' }}>
                            Perbaikan</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
